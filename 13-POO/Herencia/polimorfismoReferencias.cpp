...
int main(){
    ...
    //el slicing es un problema que ocurre cuando se asigna un objeto de una clase derivada a una variable de una clase base, en este caso el objeto de la clase Perro se asigna a la variable de la clase Animal, lo que provoca que se pierda la parte de la clase Perro que es la que tiene el metodo hacerSonido sobrescrito, por eso el resultado es el animal hace un sonido, para evitar esto tenemos que usar punteros o referencias, con punteros o referencias el resultado seria el perro ladra
